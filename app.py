...
st.markdown("""
    <style>
        ...
        /* Targets the active client header toolbar bar grid container */
        header { 
            display: none !important;
            visibility: hidden !important; 
        }
        
        /* Targets the default top right viewer badge overlays */
...
def fetch_engineered_dataset():
    # Direct access to the required file pathway
    return pd.read_csv("data/clean_bank_data.csv")

try:
    df = fetch_engineered_dataset()
    
    # 3. Sidebar Parameter Control Center (Populated entirely from production CSV variables)
    ...
    st.error("### 📁 Missing Portfolio Source Data")
    st.info("The application requires a valid database file at `data/clean_bank_data.csv` to run. Please add this dataset to your repository structure.")
