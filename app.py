...
st.markdown("""
    <style>
        ...
        .hero-banner {
            ...
            border-radius: 16px; color: white; margin-top: -40px; margin-bottom: 30px;
            box-shadow: 0 10px 25px rgba(30, 58, 138, 0.15);
        }
        .premium-card {
            background-color: #FFFFFF; border-top: 4px solid #3B82F6; padding: 24px;
        ...
        .card-label { font-size: 13px; color: #6B7280; text-transform: uppercase; letter-spacing: 1px; font-weight: 600; }
        .card-value { font-size: 32px; font-weight: 700; color: #1F2937; margin-top: 8px; }
        .image-caption-box { background-color: #FFFFFF; padding: 20px; border-radius: 12px; box-shadow: 0 4px 15px rgba(0, 0, 0, 0.04); border-left: 4px solid #1E3A8A; }
    </style>
""", unsafe_allow_html=True)

# 2. Server-Safe Data Ingestion Layer (Generates data dynamically if missing)
@st.cache_data
...
target_provinces = st.sidebar.multiselect("📍 Select Geographic Scope", options=sorted(df['Province'].unique()), default=df['Province'].unique())
target_risk = st.sidebar.multiselect("⚡ Select Underwriting Risk Tiers", options=sorted(df['Risk_Segment'].unique()), default=df['Risk_Segment'].unique())
...
st.markdown("""
    <div class='hero-banner' style='padding: 40px 30px; min-height: 160px; overflow: hidden; display: block;'>
        ...
        <h1 style='color: white; margin: 5px 0; font-size: 34px; font-weight: 800; line-height: 1.2;'>Canadian Credit Risk Portfolio Portal</h1>
        <p style='color: #E2E8F0; margin: 4px 0 0 0; font-size: 15px; font-weight: 400; opacity: 0.95;'>Institutional Asset Underwriting, Exposure Stratification, and Geospatial Mapping Architecture</p>
    </div>
""", unsafe_allow_html=True)
